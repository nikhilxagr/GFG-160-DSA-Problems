...
class Solution:
    def makeProductOne(self, arr, N):
        ...
        for i in range(N):
            if arr[i] < 0:
                neg += 1
                steps += abs(arr[i] + 1)  # -1 is the closest negative number to 1, so we need to calculate the steps to reach -1
                
            elif arr[i] > 0:
                steps += abs(arr[i] - 1)  # 1 is the closest positive number to 1, so we need to calculate the steps to reach 1
